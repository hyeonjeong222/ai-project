...
import { BookOpenText, Building2, MailPlus, ShieldCheck, UserRound, UsersRound } from "lucide-react";
...
interface Invite { id: string; email: string; role: "ADMIN" | "MEMBER" | "VIEWER"; created_at: string }
...
export function CompanySettings() {
  ...
  async function changeRole(member: Member, nextRole: "ADMIN" | "MEMBER" | "VIEWER") {
    if (!workspace || member.role === nextRole) return;
    setChangingUserId(member.user_id); setNotice(""); setError("");
    try {
      const data = await api<{ member: Member }>(`/v1/workspaces/${workspace.id}/members`, { method: "PATCH", body: JSON.stringify({ userId: member.user_id, role: nextRole }) });
      setMembers((current) => current.map((item) => item.user_id === member.user_id ? data.member : item));
      setNotice(`${data.member.name}님의 권한을 ${roleName[data.member.role]}(으)로 변경했습니다.`);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "구성원 권한을 변경하지 못했습니다."); }
    finally { setChangingUserId(""); }
  }
  ...
  return <main className="admin-page company-page"><header className="page-header"><div><p className="eyebrow">COMPANY WORKSPACE</p><h1>회사·구성원 관리</h1><p>회사마다 문서와 대화는 완전히 분리됩니다. 이곳에서 매뉴얼 관리자와 일반 구성원의 권한을 관리하세요.</p></div></header>
    ...
    <section className="company-layout"><article className="panel invite-card"><div className="panel-header"><div><p className="eyebrow">INVITE MEMBER</p><h2>구성원 초대</h2></div><MailPlus size={21} /></div><p className="muted">초대 이메일을 등록하세요. 해당 계정으로 로그인하면 자동으로 이 회사 워크스페이스에 연결됩니다.</p><form onSubmit={invite}><label className="field-label">회사 이메일<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="name@company.com" required /></label><label className="field-label">권한<select value={role} onChange={(event) => setRole(event.target.value as Invite["role"])}><option value="MEMBER">일반 구성원 — 질문·요청·매뉴얼 열람</option><option value="VIEWER">열람 전용 — 매뉴얼 원문만 열람</option>{workspace?.role === "OWNER" && <option value="ADMIN">매뉴얼 관리자 — 문서·문의함 관리</option>}</select></label>{error && <p className="form-alert error">{error}</p>}{notice && <p className="form-alert success">{notice}</p>}<button className="button primary" disabled={saving || !email.trim()}><MailPlus size={16} />{saving ? "등록 중…" : "초대 등록"}</button></form></article>
      <aside className="company-role-guide panel"><p className="eyebrow">ROLE GUIDE</p><h2>권한은 최소한으로</h2><ul><li><ShieldCheck size={16} /><div><strong>회사 관리자</strong><span>구성원과 회사 설정을 관리합니다.</span></div></li><li><Building2 size={16} /><div><strong>매뉴얼 관리자</strong><span>문서 최신화와 직원 답변을 처리합니다.</span></div></li><li><UserRound size={16} /><div><strong>일반 구성원</strong><span>AI 질문, 요청, 매뉴얼 원문 열람을 사용합니다.</span></div></li><li><BookOpenText size={16} /><div><strong>열람 전용</strong><span>매뉴얼 원문만 읽을 수 있습니다.</span></div></li></ul></aside></section>
    <section className="panel member-list"><div className="panel-header"><div><p className="eyebrow">ACTIVE MEMBERS</p><h2>현재 구성원 권한</h2></div></div>{members.map((member) => {
      const editable = member.role !== "OWNER" && (workspace?.role === "OWNER" || member.role !== "ADMIN");
      return <article key={member.user_id}><span className="member-avatar">{member.name.slice(0, 1).toUpperCase()}</span><div><strong>{member.name}</strong><small>{member.email}</small></div>{editable ? <label className="member-role-select"><span className="sr-only">{member.name} 권한</span><select value={member.role} onChange={(event) => void changeRole(member, event.target.value as "ADMIN" | "MEMBER" | "VIEWER")} disabled={changingUserId === member.user_id}><option value="MEMBER">일반 구성원</option><option value="VIEWER">열람 전용</option>{workspace?.role === "OWNER" && <option value="ADMIN">매뉴얼 관리자</option>}</select></label> : <span className={`member-role ${member.role.toLowerCase()}`}>{roleName[member.role]}</span>}{changingUserId === member.user_id && <small className="member-saving">변경 중…</small>}</article>;
    })}</section>
