...
export function DocumentUpload() {
  ...
  function chooseFile(next?: File) {
    ...
    if (next.size > 25 * 1024 * 1024) { setError("파일 크기는 25MB 이하여야 합니다."); return; }
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      const registered = await api<{ versionId: string; upload: { path: string; token: string } }>(endpoint, { method: "POST", body: JSON.stringify({ ...form, fileName: file.name, contentType, byteSize: file.size, sha256: checksum, tags: [form.category, form.department].filter(Boolean), effectiveDate: form.effectiveDate || null }) });
      const { error: uploadError } = await createSupabaseBrowserClient().storage.from("knowledge-files").uploadToSignedUrl(registered.upload.path, registered.upload.token, file, { contentType });
      if (uploadError) throw new Error("원본 파일을 업로드하지 못했습니다.");
      setStatus("QUEUED");
      await api(`/v1/document-versions/${registered.versionId}/complete`, { method: "POST", body: "{}" });
      await waitForReady(registered.versionId);
      setComplete(true);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "문서 등록에 실패했습니다."); setStatus(""); }
  }
