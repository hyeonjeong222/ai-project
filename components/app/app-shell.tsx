...
export function AppShell({ children, userEmail }: { children: React.ReactNode; userEmail: string }) {
  ...
  if (error) return <div className="full-state"><Bot size={32} /><h1>연결을 확인해 주세요</h1><p>{error}</p></div>;
  if (!workspace) return (
    <main className="workspace-setup">
      <div className="setup-card"><div className="brand-mark"><Bot size={20} /></div><p className="eyebrow">COMPANY ADMIN SETUP</p><h1>회사용 지식 공간을 개설해 주세요</h1><p>첫 개설자는 회사 관리자가 됩니다. 이후에는 구성원을 초대해 회사별 문서와 대화를 안전하게 분리합니다.</p>
  ...
  return (
    <div className="product-shell">
      ...
      <aside className={`app-sidebar ${mobileOpen ? "open" : ""}`}>
        <div className="sidebar-brand"><div className="brand-mark"><Bot size={20} /></div><div><strong>Onboard AI</strong><span>KNOWLEDGE WORKSPACE</span></div><button className="icon-button mobile-only" onClick={() => setMobileOpen(false)} aria-label="메뉴 닫기"><X size={19} /></button></div>
